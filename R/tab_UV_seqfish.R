...
tabServer_UV_seqfish <- function(id, data_path) {
    moduleServer(id, function(input, output, session) {
        ...
        srt <- reactive({
            ...
            obj <- readRDS(paste0(data_path(), "UV_seqfish/UV_seqfish_seuratObject_shiny.rds"))
            return(obj)
        })

        bulk_tb <- reactive({
            tb <- readRDS(paste0(data_path(), "UV_seqfish/UV_seqfish_pseudobulk_sum_subCellType.pub3.1.MCCD14_CellType.pub3_Condition_bin30.rds"))
            return(tb)
        })
