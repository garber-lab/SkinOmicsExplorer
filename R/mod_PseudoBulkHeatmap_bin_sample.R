modUI_PseudoBulkHeatmap_bin_sample <- function(id, allow_subset = FALSE, width_default = 8, height_default = 8, format_default = "pdf", allow_download = TRUE, show_bins_toggle = FALSE, show_bins_default = TRUE, show_bins_label = "Show transition") {
    ns <- NS(id)
    ...
    plot_panel <- wellPanel(
        ...
        bslib::input_switch(ns("show_per_sample"), "Show per sample", value = FALSE),
...
modServer_PseudoBulkHeatmap_bin_sample <- function(id, bulk_tb, bulk_meta, dataname, groupby_column = NULL, splitby_column = NULL, subsetby_columns = NULL, sample_column = NULL, show_bins_toggle = FALSE, show_bins_default = TRUE, show_plot_button = TRUE) {
    moduleServer(id, function(input, output, session) {
        ...
        collapse_other_meta <- function(md, groupby, splitby) {
            ...
            if ("bin" %in% colnames(md)) {
                used_cols <- c(used_cols, "bin")
            }
        ...
        groupby_effective <- reactive({
            if (isTRUE(input$show_per_sample)) {
                req(sample_column)
                sample_column
            } else {
                groupby_param()
        ...
        show_bins_param <- reactive({
            if (isTRUE(show_bins_toggle)) {
                isTRUE(input$show_bins)
            } else {
                isTRUE(show_bins_default)
            }
        })
        ...
        plot_heatmap <- reactive({
            plot_tb <- tb_for_plot()
            plot_md <- md_for_plot()
            req(plot_tb, plot_md, groupby_effective(), input$gene)
            if (isTRUE(input$show_per_sample)) {
                validate(need(!is.null(sample_column), "Sample column is not set."))
                validate(need(sample_column %in% colnames(plot_md), paste0("Sample column not found: ", sample_column)))
            }
            plot_md <- collapse_other_meta(plot_md, groupby_effective(), splitby_effective())
            hmap <- HeatmapPseudoBulk_bin(
                plot_tb,
                plot_md,
                genes = input$gene,
                groupby = groupby_effective(),
                splitby = splitby_effective(),
                cluster_genes = isTRUE(input$cluster_genes),
                show_bins = show_bins_param()
            )
            draw(hmap)
        })
        ...
        output$plot_heatmap_download <- downloadHandler(
            filename = function() {
                groupby_str <- groupby_effective()
                splitby_str <- splitby_effective()
